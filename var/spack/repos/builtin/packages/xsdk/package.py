...
class Xsdk(Package):
    ...
    depends_on('petsc@develop+trilinos+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64',
               when='@develop')
    depends_on('petsc@3.10.1+trilinos+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64',
               when='@0.4.0')
    ...
    depends_on('amrex@develop', when='@0.4.0 %intel')
    depends_on('amrex@develop', when='@0.4.0 %gcc')
